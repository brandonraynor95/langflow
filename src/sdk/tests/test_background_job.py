...
The HTTP layer is replaced with respx mocks so only the asyncio task
lifecycle and BackgroundJob status logic is under test.

Adapted from the BackgroundJob pattern in langflow-ai/sdk PR #1
...
def _make_task(coro) -> asyncio.Task:
    """Create a real asyncio.Task from a coroutine (requires a running loop)."""
