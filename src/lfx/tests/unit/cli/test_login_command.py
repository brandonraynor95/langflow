...
def _run_login(
    *,
    ...
    with (
        patch("lfx.cli.login._load_sdk", return_value=mock),
...
class TestLoginCommandAuthFailure:
    ...
    def test_auth_failure_exits_code_1(self):
        client = _make_client_mock()
        client.list_flows.side_effect = _FakeLangflowAuthError()
        sdk = _make_sdk_mock(client_mock=client)
        env_cfg = _make_env_cfg()
        with pytest.raises(typer.Exit) as exc_info:
            _run_login(sdk_mock=sdk, env_cfg=env_cfg)
...
class TestLoginCommandConnectionFailure:
    ...
    def test_connection_error_exits_code_1(self):
        client = _make_client_mock()
        client.list_flows.side_effect = _FakeLangflowConnectionError("ECONNREFUSED")
        sdk = _make_sdk_mock(client_mock=client)
        env_cfg = _make_env_cfg()
        with pytest.raises(typer.Exit) as exc_info:
            _run_login(sdk_mock=sdk, env_cfg=env_cfg)
...
class TestLoginCommandConfigError:
    def test_config_error_raises_exit_1(self):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("lfx.cli.login._load_sdk", return_value=sdk),
