...
class TestRenderTable:
    ...
    def test_synced_status_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="F", status="synced", path=tmp_path / "f.json", flow_id=_FLOW_ID)]
        _render_table(statuses, "test-env")

    def test_ahead_status_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="F", status="ahead", path=tmp_path / "f.json", flow_id=_FLOW_ID)]
        _render_table(statuses, "test-env")

    def test_new_status_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="F", status="new", path=tmp_path / "f.json", flow_id=_FLOW_ID)]
        _render_table(statuses, "test-env")

    def test_remote_only_status_does_not_crash(self):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="F", status="remote-only", flow_id=_FLOW_ID)]
        _render_table(statuses, "test-env")

    def test_no_id_status_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table
    ...
    def test_error_status_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="F", status="error", path=tmp_path / "f.json", detail="parse error")]
        _render_table(statuses, "test-env")

    def test_all_statuses_together_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table
        ...
        _render_table(statuses, "production")

    def test_status_with_no_path_does_not_crash(self):
        from lfx.cli.status import FlowStatus, _render_table

        statuses = [FlowStatus(name="Ghost", status="remote-only", flow_id=_FLOW_ID)]
        _render_table(statuses, "env")

    def test_status_with_no_flow_id_does_not_crash(self, tmp_path):
        from lfx.cli.status import FlowStatus, _render_table
