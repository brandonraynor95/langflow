...
def _run_push(
    ...
    *,
    ...
    with patch("lfx.cli.push._load_sdk", return_value=mock):
        push_command(
...
class TestPushCommand:
    ...
    def test_environments_file_resolves_url(self, tmp_path):
        ...
        client = _make_client_mock(create=True)
        sdk = _make_sdk_mock(client)
        with patch("lfx.cli.push._load_sdk", return_value=sdk):
            push_command(
                flow_paths=[str(p)],
                env="ci",
                dir_path=None,
    ...
    def test_missing_environments_file_exits(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        with patch("lfx.cli.push._load_sdk", return_value=sdk), pytest.raises(typer.Exit):
            push_command(
                flow_paths=[str(p)],
                env="missing",
                dir_path=None,
