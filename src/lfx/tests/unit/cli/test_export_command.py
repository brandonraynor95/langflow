...
def _run_export(
    ...
    *,
    ...
    with (
        patch("lfx.cli.export._load_sdk", return_value=mock),
...
class TestSafeFilename:
    def test_alphanumeric_unchanged(self):
        from lfx.cli.export import _safe_filename
...
class TestExportCommandRemoteFlowId:
    def test_missing_env_and_target_raises_exit_1(self):
        ...
        with patch("lfx.cli.export._load_sdk", return_value=sdk), pytest.raises(typer.Exit) as exc_info:
            export_command(
                flow_paths=[],
                output=None,
                output_dir=None,
    ...
    def test_config_error_raises_exit_1(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("lfx.cli.export._load_sdk", return_value=sdk),
