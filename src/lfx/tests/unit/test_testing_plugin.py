...
class TestLocalFlowRunner:
    def test_success_result(self):
        with patch("lfx.testing._run_sync", return_value=_SUCCESS_RAW) as mock_run:
    ...
    def test_error_result(self):
        with patch("lfx.testing._run_sync", return_value=_ERROR_RAW):
            runner = LocalFlowRunner()
    ...
    def test_env_file_loaded(self, tmp_path):
        ...
        with (
            patch("lfx.testing._run_sync", return_value=_SUCCESS_RAW),
            patch("lfx.testing._load_dotenv") as mock_load,
        ):
...
class TestAsyncLocalFlowRunner:
    async def test_success_result(self):
        with patch("lfx.testing._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW):
    ...
    async def test_error_result(self):
        with patch("lfx.testing._run_async", new_callable=AsyncMock, return_value=_ERROR_RAW):
            runner = AsyncLocalFlowRunner()
    ...
    async def test_default_timeout_used(self):
        with patch("lfx.testing._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW) as mock_run:
    ...
    async def test_env_file_loaded(self, tmp_path):
        ...
        env_file.write_text("MY_VAR=1\n")
        with (
            patch("lfx.testing._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW),
            patch("lfx.testing._load_dotenv") as mock_load,
        ):
...
class TestFixtures:
    ...
    def test_flow_runner_fixture_runs_flow(self, flow_runner):
        with patch("lfx.testing._run_sync", return_value=_SUCCESS_RAW):
