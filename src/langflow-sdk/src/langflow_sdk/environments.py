...
.. code-block:: toml
    ...
    api_key_env = "LANGFLOW_STAGING_API_KEY"   # env-var that holds the key
...
def _parse_env(raw: dict[str, Any], file_path: Path, name: str) -> EnvironmentConfig:
    ...
    if "api_key_env" in raw:
        ...
        # Allow literal key as a fallback (not recommended for production)
        api_key = raw["api_key"]
...
def get_environment(
    ...
    *,
    ...
    if name is None:
        # Try to read from config defaults
        file_path: Path | None = None
        for candidate in _candidate_paths(config_file):
            if candidate.exists():
                file_path = candidate
        ...
        if name is None:
            raise EnvironmentConfigError(
                "No environment name given and no [defaults] environment set in the config file."
            )
