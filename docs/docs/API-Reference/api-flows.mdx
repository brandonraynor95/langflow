---
title: Flow management endpoints
slug: /api-flows
---
import CodeBlock from '@theme/CodeBlock';
import exampleApiFlowsCreateFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/create-flow.sh';
import resultApiFlowsResultCreateFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/result-create-flow.json';
import exampleApiFlowsCreateFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/create-flows.sh';
import exampleApiFlowsReadFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/read-flow.sh';
import resultApiFlowsResultReadFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/result-read-flow.json';
import exampleApiFlowsReadFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/read-flows.sh';
import exampleApiFlowsReadFlows2 from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/read-flows-2.sh';
import exampleApiFlowsReadSampleFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/read-sample-flows.sh';
import exampleApiFlowsUpdateFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/update-flow.sh';
import resultApiFlowsResultUpdateFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/result-update-flow.json';
import exampleApiFlowsDeleteFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/delete-flow.sh';
import resultApiFlowsResultDeleteFlow from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/result-delete-flow.json';
import exampleApiFlowsExportFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/export-flows.sh';
import resultApiFlowsResultExportFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/result-export-flows.txt';
import exampleApiFlowsImportFlows from '!!raw-loader!@site/API-Reference/curl-examples/api-flows/import-flows.sh';


Use the `/flows` endpoint to create, read, update, and delete flows.

If you want to use the Langflow API to run a flow, see [Flow trigger endpoints](/api-flows-run).

## Create flow

Creates a new flow.

<CodeBlock language="bash">{exampleApiFlowsCreateFlow}</CodeBlock>

<details>
<summary>Result</summary>

<CodeBlock language="json">{resultApiFlowsResultCreateFlow}</CodeBlock>

</details>

## Create flows

Creates multiple new flows, returning an array of flow objects.

<CodeBlock language="bash">{exampleApiFlowsCreateFlows}</CodeBlock>

## Read flow

Retrieves a specific flow by its ID.

<CodeBlock language="bash">{exampleApiFlowsReadFlow}</CodeBlock>

<details>
<summary>Result</summary>

<CodeBlock language="json">{resultApiFlowsResultReadFlow}</CodeBlock>

</details>

## Read flows

Returns a JSON object containing a list of flows.

Retrieve all flows with pagination:

<CodeBlock language="bash">{exampleApiFlowsReadFlows}</CodeBlock>

To retrieve flows from a specific project, use the `project_id` query parameter:

<CodeBlock language="bash">{exampleApiFlowsReadFlows2}</CodeBlock>

## Read sample flows

Retrieves a list of sample flows:

<CodeBlock language="bash">{exampleApiFlowsReadSampleFlows}</CodeBlock>

## Update flow

Updates an existing flow by its ID.

This example changes the value for `endpoint_name` from a random UUID to `my_new_endpoint_name`.

<CodeBlock language="bash">{exampleApiFlowsUpdateFlow}</CodeBlock>

<details>
<summary>Result</summary>

<CodeBlock language="json">{resultApiFlowsResultUpdateFlow}</CodeBlock>

</details>

## Delete flow

Deletes a specific flow by its ID.

<CodeBlock language="bash">{exampleApiFlowsDeleteFlow}</CodeBlock>

<details>
<summary>Result</summary>

<CodeBlock language="json">{resultApiFlowsResultDeleteFlow}</CodeBlock>

</details>

## Export flows

Exports specified flows to a ZIP file.

This endpoint downloads a ZIP file containing [Langflow JSON files](/concepts-flows-import#langflow-json-file-contents) for each flow ID listed in the request body.

<CodeBlock language="bash">{exampleApiFlowsExportFlows}</CodeBlock>

<details>
<summary>Result</summary>

<CodeBlock language="text">{resultApiFlowsResultExportFlows}</CodeBlock>

</details>

## Import flows

Imports flows by uploading a [Langflow-compatible JSON file](/concepts-flows-import#langflow-json-file-contents).

To specify a target project for the flow, include the query parameter `folder_id`.
The target `folder_id` must already exist before uploading a flow. Call the [/api/v1/projects/](/api-projects#read-projects) endpoint for a list of available folders and projects.

This example uploads a local file named `agent-with-astra-db-tool.json` to a folder specified by a `FOLDER_ID` variable:

<CodeBlock language="bash">{exampleApiFlowsImportFlows}</CodeBlock>

<details>
<summary>Result</summary>

```json
[
  {
    "name": "agent-with-astra-db-tool",
    "description": "",
    "icon": null,
    "icon_bg_color": null,
    "gradient": null,
    "data": {}
  ...
  }
]
```
</details>
